...
- **Investigasi "25.4%" (§3.1):** angka ini TIDAK reproducible dari perhitungan cascade penuh (403 confirmed/1.687 verified = 23.9%) — tapi recompute pakai timestamp per-record menemukan ia reproduksi PERSIS sebagai confirm-rate batch **pre-outage** (sebelum xAI 403 kredit habis 2026-06-18): 99/389 = 25.45% ≈ 25.4%. Dicatat di `experiments/pilot06_local_models/cascade_confirm_rate_note.md` (angka saja, tanpa teks ofensif); paper §3.1 **dibiarkan** (sesuai kriteria tugas: kalau ketemu perhitungan yang mereproduksi, biarkan paper) — tapi flag untuk sesi depan: kalimat itu ambigu antara "batch pertama" vs "seluruh cascade", perlu diperjelas kalau ada waktu.
...
- **⏳ MASIH MENUNGGU BAPAK (belum bisa diotomasi):**
  ...
  5. **Keputusan strategi rasio referensi** — reference-list header note mengakui gap literal JINITA "≥80% jurnal ≤5 tahun" (19/29 lama, sekarang denominator jadi 31 setelah [30]/[31] tapi rasio belum dihitung ulang di catatan itu — lihat A16 di changelog) — perlu keputusan apakah trade beberapa referensi fondasional atau terima gap dengan disclosure.
