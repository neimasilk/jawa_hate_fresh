...
- **⚠️⚠️ GOTCHA BESAR (koreksi di sesi yang sama): yang disubmit BUKAN output builder, tapi `paper/JUTIF_Amien_Kanthi_Sijabat_2026 fixed.docx` — versi yang Bapak edit MANUAL di Word.** Saya awalnya build revisi dari markdown, sehingga 4 perubahan manual Bapak nyaris hilang. Di-diff dulu (builder-output vs fixed) → ketemu **tepat 4 perubahan, body identik**: (1) penulis ke-4 **Roby Firnando Yusuf⁴**, (2) afiliasi **⁴Jeonbuk National University, South Korea**, (3) Phone Number 08123300359, (4) Acknowledgement asli (dana DRTPM Kemdikbudristek + UBHINUS). **Keempatnya sekarang sudah dipindah ke builder + markdown**, jadi tidak hilang lagi tiap rebuild. **Pelajaran: sebelum rebuild dari md, SELALU diff dulu docx yang benar-benar disubmit vs output builder — single-source md+builder bisa bohong kalau ada edit manual di hilir.**
...
- **✅ CROSS-REF LINT (akhir sesi):** naskah dicek otomatis — 20 seksi, 8 tabel, 5 gambar, 2 persamaan; semua terdefinisi DAN tersitasi, tanpa nomor bolong, tanpa duplikat, tanpa rujukan menggantung, caption cocok dengan tag gambar. Renumbering masif (tabel/gambar/seksi/referensi) tidak merusak apa pun.
- **⏭️ NEXT — tinggal 2:**
  1. ✅ **Roby sudah ditambahkan ke metadata OJS** #6393 (Bapak, 2026-07-27) — blocker co-author internasional TUTUP.
  ...
  3. **Turnitin** via UBHINUS (maks 20%) — sedang diproses Bapak per 2026-07-27. Setelah itu: upload `paper/JUTIF_Amien_Kanthi_Sijabat_2026_R1_revised.docx` + hasil Turnitin + response letter sebagai **file BARU** di bagian revision OJS (jangan timpa file lama), lalu Bapak cek visual dulu di Word (render 2 persamaan, posisi 5 gambar / 8 tabel).
...
- **Konteks:** Bapak minta Fable review sekali lagi atas v7 sebelum lanjut Word template — apakah eksekusi sesi 10 (P1-5 restrukturisasi + lit-pass) benar-benar tuntas, dan apakah angka-angka paper masih akurat setelah semua perubahan itu. Setiap edit di bawah diverifikasi terhadap artefak ter-commit (recompute langsung dari data, bukan asumsi) sebelum diterapkan.
- **2 error faktual ditemukan + diperbaiki:**
  1. **§4.4** salah komposisi 9 sel yang lolos semua 5 detektor — kalimat lama "seven of the eight krama-sarcastic cells plus two krama-report × politik_kolektif" punya 2 kesalahan: matriks punya **9** sel krama_sarcastic (bukan 8 — Table 3 sendiri n=27=9 sel×3 generator), dan sel non-sarkastik kedua bukan dua-duanya krama_report (satu krama_report, satu krama_cold_contempt). Diperbaiki jadi "seven of the nine krama-sarcastic cells, plus one krama-report and one krama-cold-contempt cell, both targeting politik_kolektif" — diverifikasi persis vs `experiments/generation_pilot/disagreement_analysis.md` P0-3(d) (sel #17, #20-22, #24-26, #34, #36).
  2. **§4.6** klaim "the single krama instance [among hate] lacks group identity" salah — recompute majority-of-3 register langsung dari `data/labeled/bulk_v2_consensus.jsonl`: dari 158 consensus-hate, register mayoritas = **157 ngoko + 1 campur_kasar (0 krama)**; yang krama-majority di seluruh 728 record ada **2** (bukan 1), dan **keduanya non-hate** dengan target `tidak_ada`. Diperbaiki ke deskripsi yang benar.
- **2 sub-item P1-5 sesi 10 ternyata bocor** (dicatat "SELESAI" tapi belum benar-benar dieksekusi): abstract belum menyebut range multi-rater (kini "97–100% ... pooled authenticity is rater-dependent (45–91%)"); §3.3 (Method) belum mendeskripsikan instrumen validator ke-2/3 dua-kolom (baru ada di §4.7 Results) — kini ditambah paragraf baru di akhir §3.3. **Lesson:** hasil eksekutor perlu diverifikasi item-per-item terhadap spesifikasi asli, bukan diasumsikan benar dari catatan "selesai" di changelog.
- **Fix lain:** denominator §1 "74/8,269" (raw termasuk duplikat) → "74 of 7,823 deduplicated tweets" (match `pilot02_llm_jawa_filter/report.md`); speaker count 80M→84M (konsisten dgn Table 1); cross-ref stale §2.3 "(§4.5)"→"(§4.6)"; pointer eksplisit Fig.1/Fig.2 di prosa; gloss SARA (suku/agama/ras/antargolongan) di first-use §3.2; "LLM" dijabarkan jadi "large language models (LLMs)" di first-use §1; sitasi [30]/[31] ditambah ke Gemma3/GPT-OSS; format referensi (>3 penulis → et al. untuk [1][17][20][27][29], DOI ditambah [10][21], tahun eksplisit [2][12]); path repo internal dihapus dari §4.1 prosa (→ "supplementary materials"); "gitignored"→"excluded" di §4.9; §5 Conclusion disamakan dgn abstract (range multi-rater, bukan 97% tunggal).
- **Table 1 diverifikasi 100%:** semua 10 baris (Javanese + 9 bahasa lain) dicocokkan langsung vs `D:\documents\twitter\datasets\dvi_scores.csv` — SEMUA cocok persis (speaker, confirmed tweets, confirmed rate), termasuk total corpus 1.419.641 tweet (cross-check ke draft paper sister-project lain yang independen). Table 1 paper **tidak diubah** (sudah benar); snapshot reproducibility dibuat di `paper/external/dvi_table1_snapshot.csv` + `README.md` (provenance, metode 1-kalimat, izin per D20).
- **Investigasi "25.4%" (§3.1):** angka ini TIDAK reproducible dari perhitungan cascade penuh (403 confirmed/1.687 verified = 23.9%) — tapi recompute pakai timestamp per-record menemukan ia reproduksi PERSIS sebagai confirm-rate batch **pre-outage** (sebelum xAI 403 kredit habis 2026-06-18): 99/389 = 25.45% ≈ 25.4%. Dicatat di `experiments/pilot06_local_models/cascade_confirm_rate_note.md` (angka saja, tanpa teks ofensif). **Follow-up Fable (sesi sama):** paper §3.1 **DIKOREKSI ke 23.9% (403/1.687)** — kalimatnya mendeskripsikan "the cascade's cloud confirmation rate" (seluruh kaskade), jadi mengutip rate batch pra-outage adalah kelas error yang sama dengan bug denominator P0-1 (rate batch dikutip sebagai rate keseluruhan run). Note file jadi provenance untuk kedua angka.
...
- **✅ KEPUTUSAN BAPAK (2026-07-09, sesi sama — 6 item review dijawab):**
  ...
  5. **Rasio referensi: "sesuaikan"** — Bapak minta gap dipersempit. **SELESAI (2026-07-09, lit-pass ke-3):** +7 referensi jurnal 2023–2025 ditambah ([32]–[38]: Jahan & Oussalah *Neurocomputing* 2023; Sharma et al. *ACM TALLIP* 2025; Sulistyo et al. *JOIV* 2025; Hidayatullah et al. *PeerJ Comput. Sci.* 2023; Pavlyshenko & Stasiuk *Int. J. Comput.* 2025; Törnberg *Soc. Sci. Comput. Rev.* 2024; Oprea & Bâra *J. King Saud Univ. – Comput. Inf. Sci.* 2025 — semua diverifikasi via Crossref API sebelum ditulis), masing-masing dipasang di klaim spesifik yang genuinely didukung (§1 ×5, §2.4 ×1, §3.1 ×1), bukan cluster padding. Referensi total 31→38. Rasio: recency 21/31 (68%) → 28/38 (74%); jurnal 11/31 (35%) → 18/38 (47%); keduanya (aturan literal JINITA) **8/31 (26%) → 15/38 (39%)**. Kepatuhan literal 80% tetap tidak tercapai tanpa membuang fondasi (Errington/Krippendorff/anchor lama) — disclosure di header note dipertahankan, angka di-update. Tidak ada referensi lama yang dihapus/diganti.
...
**🆕 SESI 10 (lanjutan #2, 2026-07-07) — Publish-readiness review + P1:**

- Bapak minta review+kritik jujur atas paper (layak publish? perlu ilustrasi?). Jawaban: sains inti kuat (taksonomi novel, bukti kelangkaan ganda, kejujuran radikal soal IRR rendah), tapi ada gap struktural/editorial nyata — semua diperbaiki sesi ini, bukan cuma didiskusikan.
- **P1-5 restrukturisasi:** diagnosis multi-validator (~700 kata, sebelumnya terkubur di Limitation (1)) dipindah ke Results subsection baru **§4.7 "Multi-validator authenticity: instrument and sociolinguistic effects"** (Table 5 rate+α pairwise, Table 6 raw vs harmonized α). Limitation (1) (kini §4.8) diringkas ~100 kata, menunjuk balik ke §4.7. Limitations lama §4.7→§4.8, Ethics §4.8→§4.9, semua cross-reference internal diverifikasi konsisten.
- **P1-6 fix:** "documented Javanese sociolect" (klaim Chindo) → "anecdotally reported" — itu anekdot Bapak, bukan literatur.
- **Wording fix:** "five independent hate-speech detectors" → "five hate-speech detector models" (di §1; abstrak sudah aman). Ditambah kalimat eksplisit di §3.4: kelima detektor berbagi 1 prompt/taksonomi — independen sbg vendor, BUKAN independen sbg kriteria penilaian. Sebelumnya caveat ini cuma 1 kalimat di limitation terakhir (#5 dari 6), padahal kata "independent" dipakai berat di abstrak/intro.
- **Lit-pass ke-2:** cari + verifikasi (via Crossref API, bukan cuma snippet search) 3 referensi baru sangat relevan & baru (2026): **[27]** Pamungkas dkk. (multimodal Indonesian hate speech, J. Computational Social Science — follow-up penulis yg sudah disitasi [13]/[14]), **[28]** Razak dkk. (XAI audit detektor hate speech, **terbit di JINITA sendiri**), **[29]** SEAHateCheck (Ng dkk., HateCheck-style test suite utk Indonesia/Tagalog/Thai/Vietnam — TIDAK cover Jawa, justru menguatkan klaim novelty paper ini, bukan mengancamnya). Catatan header referensi kini jujur mengakui: aturan literal JINITA "≥80% jurnal terbit ≤5 tahun" TIDAK terpenuhi (19/29 referensi ≤5 tahun, 8/29 jurnal+≤5-tahun), dengan daftar eksplisit referensi fondasional yang jadi pengecualian disengaja (sosiolinguistik Jawa, metodologi Krippendorff, anchor dataset hate speech Indonesia awal).
- **2 figure baru** (`paper/figures/make_figures.py`, generate dari data Table 3/4 yang sudah ada, 0 eksperimen baru): **Fig. 1** diagram taksonomi 4 niche (dikelompokkan register ngoko/krama, tint hangat/dingin) di §2.3; **Fig. 2** heatmap deteksi (Table 4) dengan baris krama-sarcastic di-outline merah di §4.4 — blind-spot jadi kelihatan sekali pandang, bukan cuma angka di tabel. Warna dari palet `dataviz` skill yang tervalidasi (sequential blue utk magnitude, bukan hex sembarang). **Dicek dulu vs artikel JINITA asli** (`paper/jinita_guidelines/sample_article.pdf`): paper 8 halaman di sana pakai 4 figure + 11 tabel — jadi figure itu norma JINITA, bukan opsional/dekoratif.
- Front-matter version banner di-bump ke **v7** (2026-07-07), referensi 24→29, deskripsi update.
- **Belum dikerjakan:** Word template conversion (JINITA Google Doc template — beda tooling, sesi terpisah), Acknowledgements masih placeholder, versi blind (anonim) belum dibuat. Semua next-step lain dari sesi sebelumnya (ref [4] page check, P2-8 opsional) masih berlaku. Sebelumnya sesi 10 awal: E1 hasil masuk — IRR native rendah (α 0.336, di bawah ambang) + koreksi kredensial Daniel (bukan penutur asli). Sebelumnya: Reframe paper (D20), sesi 9; Demo webapp commit `5b4e5a0` (sesi 8); Paper v4 + validasi native (sesi 7).
...
**Hasil kunci:**
- **Pool 332 → 735** (held-out 586 | prompt-iter 149). Grok confirm-rate di pass2-keeps cuma **25.4%** (1687→+304) → local pre-screen over-keep (temuan cascade-design). *[Koreksi 2026-07-09: 25.4% = rate batch pra-outage saja (99/389); rate cascade penuh = 23.9% (403/1.687) — lihat `experiments/pilot06_local_models/cascade_confirm_rate_note.md`; paper §3.1 kini pakai 23.9%.]* **Dump 12.7K habis → 735 ≈ ceiling** sumber ini (estimasi ~950 optimis).
