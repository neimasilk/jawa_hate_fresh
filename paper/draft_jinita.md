...
> **JINITA-conformant draft v7 (2026-07-07). Diagnostic-suite framing (D20), multi-validator results (D21), and a publish-readiness pass.** Replaces v4 (generation-headline framing). Reframe rationale: generation is repositioned as the stimulus-construction *method* in service of a detection-blind-spot *diagnosis* (the headline contribution), rather than as the contribution itself — this closes the dual-use framing risk and better matches what the evidence supports. Taxonomy (§2) and scarcity evidence (§3.1) are unchanged from v4. Added since v5: a second/third native validator pass (§4.7, Table 5/6), a narrowed §4.5 claim informed by that pass, two figures (Fig. 1 taxonomy, Fig. 2 detection heatmap), and 3 additional references from a second lit-pass. All numbers from experiments: `experiments/generation_pilot/validation_result.md`, `experiments/generation_pilot/multivalidator_result.md`, `experiments/generation_pilot/disagreement_analysis.md`, `experiments/generation_pilot/RESULTS_probe.md`, `experiments/register_probe/FINDINGS.md`, `experiments/pilot05_bulk_labeling/report.md`. Reference list: 29 total (see change log appendix for both lit-passes); only [4]'s exact page range needs a final manual check before submission (publisher vs. aggregator discrepancy), and the reference-list header note flags a residual gap against JINITA's literal "≥80% journal papers ≤5 years" rule.
...
[27] E. W. Pamungkas, A. F. Syafiandini, D. Purworini, W. Widayat, D. G. P. Putri, I. Amal, and M. Song, "Reading between modalities: multimodal hate speech detection in low-resource Indonesian social media," *J. Computational Social Science*, vol. 9, no. 2, Art. 39, 2026, doi: 10.1007/s42001-026-00473-4.
