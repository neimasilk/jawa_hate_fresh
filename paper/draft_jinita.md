...
> *IEEE numbered style. Verified 2026-07-06 (lit-pass, D20) via ACL Anthology / arXiv / Crossref / IEEE Xplore / DBLP / publisher records — no fabricated details per JINITA Gen AI policy. Two references present in an earlier draft (Gwet 2008; AI Singapore SEA-LION) were removed because they were never actually cited in the body text of this version of the paper (a leftover from an earlier draft's scope) — every reference below is cited at least once. [4]'s page range (pp. 65–69, a prior draft flagged a publisher/aggregator discrepancy against 461–465) was confirmed 2026-07-07 directly from the paper's own PDF page footers (pages printed 65 through 69) — the aggregator's figure was a metadata error, not a citation ambiguity. **[27]–[29] added 2026-07-07** (second lit-pass, publish-readiness review) after a targeted search for directly-relevant recent work; all three verified via the Crossref API (DOI, authors, volume/issue/pages fetched programmatically, not taken from search-snippet text) before citing. **[30]–[31] added 2026-07-09** (model citations for Gemma 3 and gpt-oss, previously used in §3.2/§3.4 without citation; both verified via the arXiv API). **[32]–[38] added 2026-07-09** (third lit-pass, at the first author's request, to narrow the recency/journal-ratio gap; all verified via the Crossref/arXiv API before citing) — seven recent (2023–2025) journal articles appended at points they genuinely support: a hate-speech-detection NLP survey [32] and a low-resource-language hate-speech survey [33] alongside the opening claim (§1); a 2025 Indonesian-regional-language parallel-corpus paper [34] alongside the under-resourced-NLP claim (§1); a Javanese-Indonesian-English code-mixed corpus paper [35] alongside the code-mixing claim (§1); an LLM-based text-classification data-augmentation paper [36] alongside the GPT-augmentation claim (§1); a 2024 journal study on LLMs outperforming human coders at annotation [37] alongside the LLM-rater method description (§3.1); and a 2025 LLM-as-judge sarcasm-detection paper [38] alongside the irony-recognition claim (§2.4). **JINITA's reference rule ("minimum of 20 references, primarily with a minimum of 80% to journal papers published last 5 years ago") is still not literally met, though the gap is narrower**: 28/38 references are within 5 years, and 18/38 are journal articles, of which 15/38 (≈39%) are both — short of 80% by a strict reading, but up from 8/31 (≈26%) before this pass. The residual shortfall is concentrated in intentionally foundational citations (Javanese sociolinguistics [6], [25], [26]; Krippendorff's canonical alpha methodology [15]; early Indonesian hate-speech-dataset anchors [5], [9], [20], [21], [23]) that predate the 5-year window by design rather than by omission; literal 80% compliance would require discarding these foundations, which the authors judge is not in the paper's interest.*
...
[32] M. S. Jahan and M. Oussalah, "A systematic review of hate speech automatic detection using natural language processing," *Neurocomputing*, vol. 546, Art. 126232, 2023, doi: 10.1016/j.neucom.2023.126232.

[33] D. Sharma, T. Nath, V. Gupta, and V. K. Singh, "Hate speech detection research in South Asian languages: A survey of tasks, datasets and methods," *ACM Trans. Asian Low-Resour. Lang. Inf. Process.*, vol. 24, no. 3, pp. 1–44, 2025, doi: 10.1145/3711710.

[34] D. A. Sulistyo, A. P. Wibawa, D. D. Prasetya, F. A. Ahda, I. N. G. A. Astawa, and F. A. Dwiyanto, "Multilingual parallel corpus for Indonesian low-resource languages," *JOIV: Int. J. Informatics Visualization*, vol. 9, no. 5, p. 2176, 2025, doi: 10.62527/joiv.9.5.3412.

[35] A. F. Hidayatullah, R. A. Apong, D. T. C. Lai, and A. Qazi, "Corpus creation and language identification for code-mixed Indonesian-Javanese-English tweets," *PeerJ Comput. Sci.*, vol. 9, Art. e1312, 2023, doi: 10.7717/peerj-cs.1312.

[36] B. Pavlyshenko and M. Stasiuk, "Using large language models for data augmentation in text classification models," *Int. J. Comput.*, vol. 24, no. 1, pp. 148–154, 2025, doi: 10.47839/ijc.24.1.3886.

[37] P. Törnberg, "Large language models outperform expert coders and supervised classifiers at annotating political social media messages," *Soc. Sci. Comput. Rev.*, vol. 43, no. 6, pp. 1181–1195, 2024, doi: 10.1177/08944393241286471.

[38] S.-V. Oprea and A. Bâra, "LLM-as-a-judge for sarcasm detection using supervised fine-tuning of transformers," *J. King Saud Univ. – Comput. Inf. Sci.*, vol. 37, no. 10, Art. 357, 2025, doi: 10.1007/s44443-025-00379-7.
...
| F3 | §3.3 | **Factual correction, confirmed by the first author (2026-07-09):** "(native Javanese speaker from East Java, academic background in Javanese linguistics)" → "(native Javanese speaker from East Java)". The "academic background in Javanese linguistics" credential was never true — a fabricated phrase from an earlier drafting session, the same error class as the earlier Daniel-credential correction (v5→v6). Native-speaker status is the accurate and sufficient qualification for the authenticity-refereeing role. |
| L1 | References | **Third lit-pass (2026-07-09, same day, at the first author's request):** appended **7 new references ([32]–[38])**, all journal articles published 2023–2025 and verified via the Crossref API (DOI, authors, container-title, volume/issue/pages fetched programmatically, not from search-snippet text) before citing, each placed at a point it genuinely supports rather than as a citation cluster: a hate-speech-detection NLP survey [32] (Jahan & Oussalah, *Neurocomputing* 2023) and a low-resource-language hate-speech survey [33] (Sharma et al., *ACM TALLIP* 2025) alongside §1's opening claim; an Indonesian-regional-language parallel-corpus paper [34] (Sulistyo et al., *JOIV* 2025, explicitly covering Javanese) alongside §1's under-resourced-NLP claim; a Javanese-Indonesian-English code-mixed-corpus paper [35] (Hidayatullah et al., *PeerJ Comput. Sci.* 2023) alongside §1's code-mixing claim; an LLM-based text-classification data-augmentation paper [36] (Pavlyshenko & Stasiuk, *Int. J. Comput.* 2025) alongside §1's GPT-augmentation claim; a 2024 journal study on LLMs outperforming human coders at annotation [37] (Törnberg, *Soc. Sci. Comput. Rev.*) alongside §3.1's LLM-rater method description; and a 2025 LLM-as-judge sarcasm-detection paper [38] (Oprea & Bâra, *J. King Saud Univ. – Comput. Inf. Sci.*) alongside §2.4's irony-recognition claim. No existing reference was removed, replaced, or renumbered. Reference count 31 → 38. Ratio improved: recency (≤5 years) 21/31 (68%) → 28/38 (74%); journal-article share 11/31 (35%) → 18/38 (47%); both criteria (JINITA's literal "journal AND ≤5 years" rule) 8/31 (26%) → 15/38 (39%). The reference-list header note was updated accordingly; literal 80% compliance remains unmet (and, per the header note, is not pursued at the cost of dropping foundational citations). |
