...
| v2 | 2026-06-10 | **0.763 (+0.229)** ✅ CI [0.624, 0.879] | — (Kimi dropped, D15) | **KEEP — prompt final untuk bulk.** Disagreement 21→12, flip sehat (F→T: ds 1, grok 2), hate-rate stabil (ds 19%, grok 31%). Residu 12 = ambigu genuin (meta-komentar ttg diskriminasi, kutipan, perbandingan positif antar-etnis) → TIDAK dikejar via prompt (overfit risk ke pool 149); jadi bahan codebook + diukur ulang di held-out set saat bulk. |

**Kesimpulan Pilot #3 (2026-06-10):** pertanyaan inti terjawab dalam 2 iterasi terfokus — α ds+grok **0.534 → 0.763** murni via perbaikan definisi prompt (profanity≠hate, slur-identitas-ke-individu=hate). Keduanya koreksi prinsipil (kontradiksi internal v0), bukan tuning pool-specific. Iterasi v3+ di pool yang sama berisiko overfit → stop, validasi ulang di held-out saat bulk.
