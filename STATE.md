...
**Stage:** Pilot #1, #2, #1b DONE → C3 terjawab YELLOW (α=0.384 non-degenerate). Next: scale filter pool besar ATAU iterasi prompt (Pilot #3)
...
3b. **Pilot #1b — C3 re-test** ✅ DONE 2026-06-08
   - 3-LLM (prompt v0 sama) di 24 teks hot-Jawa → **α=0.384 non-degenerate**, gate YELLOW. C3 terjawab (consensus moderat). Kimi = noise utama.
   - **NEXT (pilih):** (a) scale filter haipradana ke pool besar (~12.7K → ~460 hot) lalu C3 ulang untuk α robust (CI sempit); ATAU (b) iterasi prompt Pilot #3 dulu untuk angkat α. Rekomendasi: (a) karena CI lebar = masalah n, bukan prompt.
