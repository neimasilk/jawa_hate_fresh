...
**Stage:** Pilot #1, #2, #1b, #3, **#6 DONE**. Pilot #6 hasil: **qwen3:14b lokal α 0.660 LOLOS** (consensus deepseek+lokal viable tanpa xAI); SEA-LION α 0.422 GAGAL. Pilot #5 bulk tertahan (xAI habis @4351/12703 → 332 hot-Jawa cukup). **Next: keputusan Bapak vendor mix final** (rekomendasi: deepseek+qwen3) → jalankan bulk labeling.
...
6. **Pilot #5 — Bulk labeling (produksi pertama)** ⏸️ PAUSED (xAI habis 2026-06-10)
   - Filter berhenti @4351/12703 (2.225 err 403) → **332 hot-Jawa (190 hate orig)** didapat = 2.2× pool lama, cukup untuk dataset pertama.
   - Deliverable tetap: **held-out validation α** (anti-overfit), `data/labeled/bulk_v2_consensus.jsonl` (dataset v1), `bulk_v2_disagreement.jsonl` (bahan codebook), profil taksonomi.
   - Infra siap lanjut dengan vendor mix apapun: `run_filter.py` (`FILTER_VENDOR=ollama`) + `run_bulk.py` (`BULK_VENDORS=deepseek,ollama`) sudah diparametrize 2026-06-11; error 403 dihitung transient (retry saat rerun).
